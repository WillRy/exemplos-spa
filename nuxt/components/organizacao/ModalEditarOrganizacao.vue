    ...
    <BaseModal :aberta="organizacao" @onClose="fecharModal" @onOpen="carregarFormulario">
        ...
        <template #body>
            ...
            <form @submit.prevent="submit" v-if="!loadingDados">
                ...
                <div class="row">
                    <div class="col">
                        <BaseSelectAjax label="Tags" placeholder="Pesquise as tags" v-model="tags" track-by="id"
                            text-by="nome" :options="resultadoPesquisaTag.lista" @search-change="pesquisarTag"
                            noOptions="Pesquisa por tags" :empty="true" :remover="true" :multiple="true">
...
const carregarFormulario = async function () {
    ...
    const response = await $fetchApi(`/organizacao/${props.organizacao.id}`)
...
const pesquisarTag = function (pesquisa) {
    $fetchApi(`/tag`, { params: { pesquisa: pesquisa } }).then((response) => {
        resultadoPesquisaTag.lista = response.data.data
    })
}
...
const submit = async function () {
    try {
        ...
        await $fetchApi(`/organizacao/${props.organizacao.id}`, {
            method: 'PUT',
            body: data
        })
        ...
        const errors = backendToastError(e, 'Erro ao salvar organização')
        setErrors(errors)
    } finally {
        loading.value = false
    }
